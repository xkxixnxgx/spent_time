...
class TrackAddForm(FlaskForm):
    # render_kw параметр в wtf, в котором можно указать то, что будет добавлено при отрисовке формы.
    # Мы добавляем класс из bootstrap из строки input class="form-control"
    track_name = StringField('New name of the track', render_kw={"class": "form-control"})
    total_time = StringField('Time, hours of the track', render_kw={"class": "form-control"})
    track_add = SubmitField('Add track', render_kw={"class": "btn btn-success"})


class TrackEditForm(FlaskForm):
    track_name = StringField('Edit your track', render_kw={"class": "form-control"})
    total_time = StringField('New time, hours', render_kw={"class": "form-control"})
